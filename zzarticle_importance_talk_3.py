...
def main():
    ...
    for fapage in preloadingGen:
        if fapage.namespace()==1:
            fapage=pywikibot.Page(faSite,fapage.title().replace('بحث:',''))
        pywikibot.output('------------'+fapage.title()+'------------')
        if fapage.namespace()!=0:
            pywikibot.output('\03{lightred}Bot only works on article namespace! so it is passed!\03{default}')
            continue
        ...
        try:
            ...
            pass
        if Overall_score:
            bot_msg='ربات:درجه‌بندی [[وپ:آفلاین|ویکی‌پروژه آفلاین]]> امتیاز='+score_num+'؛کیفیت='+quality_variable+'؛حجم مؤثر='+pagesize+'؛حجم مقاله='+main_page_size+'؛بازدید='+hitcount_result+'؛میانویکی='+Interwiki_num
            ...
            try:
                ...
                if '{{ویکی‌پروژه نسخه آفلاین' in talk_text:
                    ...
                    pywikibot.output('\03{lightgreen}The template is replaced\03{default}')
                    try:
                       pywikibot.output('\03{lightgreen}+'+Overall_score+'\03{default}')
                       Talk_fapage.put(talk_text.strip(),bot_msg)
                       continue
                    except:
                        errors(fapage)
                        pywikibot.output('\03{lightred}An error happened so it will be pass!\03{default}')
                        continue

            except pywikibot.IsRedirectPage:
                pywikibot.output("\03{lightyellow}The page %s is a redirect, going to get target page.\03{default}" % fapage.title())    
                ...
                try:
                    ...
                    pywikibot.output("\03{lightblue}The page %s doesn't exist, skip!\03{default}" % Talk_fapage.title())
                    errors(Talk_fapage)
                    continue
            except:
                talk_text='\n'
