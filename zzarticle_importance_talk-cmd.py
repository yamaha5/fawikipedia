...
def main(faname,username):
    PageTitles = []
    result={}
    faPages=[wikipedia.Page(faSite,faname)]
    for fapage in faPages:
        if fapage.namespace()!=0:
            result['msg']=u'ربات فقط در فضای نام اصل (مقاله) کار می کند.'
        ...
        try:
            ...
            pass
        if Overall_score:
            bot_msg=score_num+u'؛کیفیت='+quality_variable+u'؛حجم مؤثر='+pagesize+u'؛حجم مقاله='+main_page_size+u'؛بازدید='+hitcount_result+u'؛میانویکی='+Interwiki_num
            bot_msg=u'ربات:درجه‌بندی [[وپ:آفلاین|ویکی‌پروژه آفلاین]] ('+username+u') > امتیاز='+En2Fa_num(bot_msg)
            Talk_fapage=wikipedia.Page(faSite,u'بحث:'+fapage.title())
            try:
                talk_text=Talk_fapage.get()+u'\n'
                if Overall_score in talk_text:
                    result['msg']=u'رتبه‌بندی به‌روز است و نیاز نیست از نو به‌روز شود.'
                    print json.dumps(result)
                    continue
                    ...
                    if u'{{ویکی‌پروژه نسخه آفلاین' in talk_text:
                        ...
                        try:
                           ...
                           continue
                            ...
                            result['msg']=u'خطائی رخ داد!۲'
                            print json.dumps(result)
                            continue
                except:
                    result['msg']=u'خطائی رخ داد!۳'
                    print json.dumps(result)
                    continue
            except:
                talk_text=u'\n'
