...
class PageArchiver(object):
    ...
    def analyze_page(self):
        ...
        for t in oldthreads:
            ...
            try:
                if ('{{بسته' in t.content and '{{پایان' in t.content) or ('{{بستن' in t.content and '{{پایان' in t.content) or '{{بسته|' in t.content or '{{بستن|' in t.content:
                    why=('duration', -1)
                if 'ویکی‌پدیا:درخواست انتقال' in self.page.title():
                    test_content=t.content
                    test_content=test_content.replace('{{شد','').replace('{{نشد','').replace('نشد}}','').replace('شد}}','').replace('{{وضعیت|شد}}','').replace('{{وضعیت|نشد}}','')
                    if '{{وضعیت|}}' in t.content:
                        why=t.should_be_archived(self)
                    if test_content==t.content:
                        why=t.should_be_archived(self)
                    else:
                        why=('duration', -1)
                if why:
...
def main(*args):
    ...
    for arg in local_args:
        ...
        if option in ('file', 'filename'):
            ...
            pagename = value
        elif option == 'namespace' or option == 'ns':
            namespace = value
